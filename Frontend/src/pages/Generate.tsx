...
import { colorSchemes, dummyThumbnails, type AspectRatio, type IThumbnail, type ThumbnailStyle } from '../../public/assets/assets';
...
const Generate = () => {
  ...
  const [styleDropdownMenu , setStyleSropdownOpen]=useState(false)
  ...
  const fetchThumbnail =async()=>{
    if(id){
      ...
      setStyle(thumbnail.style as ThumbnailStyle)
        ...
        <div className='grid lg:grid-cols-[400px_1fr] gap-8'>
          ...
          <div className={`space-y-6 ${id && 'pointer-events-none'}`}>
            ...
            <div className='space-y-5'>
              ...
              {/* STYLESELECTOR */}
              <StyleSelector value={style} onChange={setStyle} isOpen={styleDropdownMenu} setIsOpen={setStyleSropdownOpen}/>
              {/* COLORSCHEMESELECTOR */}
              <ColorSchemeSelector value={colorScheme} onChange={setColorScheme}/>

              {/* DETAILS */}
              <div className='space-y-2'>
                <label className='block text-sm font-medium'>Additional prompts <span>(optional)</span></label>
                <textarea 
                value={additionalDetails} 
                onChange={(e)=>setAdditionalDetails(e.target.value)}
                rows={3}
                placeholder='Add any specific elements, mood , or style preferences...'
                className='w-full px-4 py-3 rounded-lg border border-white/10 bg-white/6 text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-pink-500 resize-none'></textarea>
              </div>
            </div>

            {/* BUTTON */}
            {!id && (
              <button onClick={onGenerate} className='text-[15px] w-full py-3.5 rounded-xl font-medium  bg-linear-to-b from-pink-500 to bg-pink-600 hover:from-pink-700 disabled:cursor-not-allowed transition-colors'>
